...
const GeneralSettings = () => {
    ...
    const handleSaveSalarySettings = async (user) => {
        try {
            ...
            showNotification("Paramètres salaire enregistrés.", "success");
            fetchUsers();
        } catch (err) {
    ...
    return (
        <Paper elevation={3} sx={{ p: 2 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Paramètres par utilisateur : plafond de salaire (prime), autoriser le dépassement, et pourcentage appliqué à la marge.
              Le pourcentage global ci-dessus sert de valeur par défaut si aucun % n'est défini sur l'utilisateur.
            </Typography>
            <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', mb: 2, flexWrap: 'wrap' }}>
            ...
            <TableContainer component={Paper} variant="outlined">
                <Table size="small">
                    ...
                    <TableBody>
                      {users.map(user => {
                        const percentValue = (user.salaryPercentageOfMargin === null || user.salaryPercentageOfMargin === undefined)
                          ? ''
                          : String(Number(user.salaryPercentageOfMargin) * 100);
                        const maxSalaryValue = (user.maxSalary === null || user.maxSalary === undefined) ? '' : String(user.maxSalary);
                        return (
                          <TableRow key={user._id}>
                            ...
                            <TableCell align="right">
                              <TextField
                                ...
                                onChange={(e) => {
                                  ...
                                  updateUserField(user._id, { salaryPercentageOfMargin: Number.isFinite(asNumber) ? (asNumber / 100) : null });
                                }}
                            ...
                            <TableCell>
                              <Chip label={user.isActive ? "Actif" : "Désactivé"} color={user.isActive ? "success" : "error"} size="small" />
                            </TableCell>
                            <TableCell align="center">
